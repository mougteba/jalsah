...
export default function StagePage() {
  ...
  // دعم الخطوات الجديدة أو الرابط القديم
            ...
            {!submitted ? (
              <>
                ...
                <button onClick={handleSubmit}
                  disabled={Object.keys(answers).length < stage.questions.length}
                  style={{
                    background: Object.keys(answers).length < stage.questions.length ? "#1a1a1a" : "#C9A84C",
                    color: Object.keys(answers).length < stage.questions.length ? "#444" : "#000",
                  }}
              ...
              <div className="space-y-4">
                ...
                {stage.questions.map((q, i) => (
                  <div key={i} style={{background:"#111", border:"1px solid #222"}} className="rounded-xl p-4 space-y-2">
                    ...
                    {answers[i] !== q.answer && (
                      <p className="text-xs text-gray-500">الإجابة الصحيحة: <span style={{color:"#C9A84C"}}>{q.answer}</span></p>
                    )}
                  </div>
                ))}
