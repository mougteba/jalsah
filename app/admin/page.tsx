...
import { Plus, ClipboardList, Users, Settings, Brain, Sparkles, Loader } from "lucide-react";
...
function AddStagePage() {
  ...
  const handleAiFill = async () => {
    ...
    try {
      ...
      setAiError("تعذّر استخراج المحتوى، تحقق من الرابط وحاول مجدداً");
    }
    setAiLoading(false);
  };
  ...
  return (
    <div className="px-4 pt-6 space-y-4 pb-6">
      ...
      <div style={{background:"#111", border:"1px solid #1e1e1e"}} className="rounded-2xl p-5 space-y-4">
        ...
        <div className="space-y-2">
          <p style={{color:"#444"}} className="text-xs">المسار</p>
        ...
        {steps.map((step, i) => (
          <div key={i} style={{background:"#0e0e0e", border:"1px solid #1a1a1a"}} className="rounded-xl p-3 space-y-2">
            <div className="flex items-center justify-between">
              ...
              {steps.length > 1 && (
                <button onClick={() => removeStep(i)} className="text-xs text-red-500">حذف</button>
              )}
            </div>
            <input value={step.title} onChange={(e) => updateStep(i, "title", e.target.value)}
              placeholder="عنوان الخطوة (مثال: مقدمة في الذكاء الاصطناعي)"
      ...
      <div style={{background:"#111", border:"1px solid #1e1e1e"}} className="rounded-2xl p-5 space-y-4">
        ...
        {questions.map((q, i) => (
          <div key={i} style={{background:"#0e0e0e", border:"1px solid #1e1e1e"}} className="rounded-xl p-4 space-y-3">
            ...
            <div className="flex gap-2">
              {(["mcq","truefalse"] as const).map((type) => (
                <button key={type} onClick={() => updateQuestion(i, "type", type)}
                  style={q.type === type ? {background:"#C9A84C", color:"#000"} : {border:"1px solid #222", color:"#555"}}
            ...
            <div className="space-y-1">
              <p style={{color:"#444"}} className="text-xs">الإجابة الصحيحة</p>
...
function StagesPage() {
  ...
  const saveEdit = async (stageId: string) => {
    await updateStage({
      ...
      phase: editData.phase || undefined,
  ...
  return (
    <div className="px-4 pt-6 space-y-3 pb-6">
      {trackList.map((track) => {
        const trackStages = stages?.filter(s => s.track === track.id)
                        ...
                        <div className="space-y-2">
                          ...
                          {editData.questions.map((q: any, i: number) => (
                            <div key={i} style={{background:"#111", border:"1px solid #222"}} className="rounded-lg p-3 space-y-2">
                              ...
                              <div className="flex gap-2">
                                {(["mcq","truefalse"] as const).map(type => (
                                  <button key={type} onClick={() => updateQ(i, "type", type)}
                                    style={q.type === type ? {background:"#C9A84C", color:"#000"} : {border:"1px solid #222", color:"#555"}}
                                    className="px-2 py-1 rounded-full text-xs">
                                    {type === "mcq" ? "متعدد" : "صح/خطأ"}
                                  </button>
                                ))}
                              </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-1.5 shrink-0 mr-2">
                          <button onClick={() => startEdit(stage)}
                            style={{border:"1px solid #1a2a3a", color:"#60a5fa"}}
                            className="text-xs px-2 py-1 rounded-lg">تعديل</button>
                          <button onClick={() => toggleLock({ stageId: stage._id, locked: !stage.adminLocked })}
                            style={stage.adminLocked
                              ? {border:"1px solid #1a3a1a", color:"#4ade80"}
                              : {border:"1px solid #2a2500", color:"#C9A84C"}}
                            className="text-xs px-2 py-1 rounded-lg">
                            {stage.adminLocked ? "فتح" : "قفل"}
                          </button>
        ...
        <div key={u._id} style={{background:"#111", border:"1px solid #1e1e1e"}} className="rounded-xl p-4 space-y-3">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <button onClick={() => banUser({ userId: u._id, banned: !u.banned })}
              style={{border:"1px solid #2a1500", color:"#f97316"}}
              className="text-xs px-3 py-1.5 rounded-lg flex-1">
              {u.banned ? "رفع الحظر" : "حظر"}
            </button>
            <button onClick={() => deleteUser({ userId: u._id })}
              style={{border:"1px solid #2a1a1a", color:"#ef4444"}}
              className="text-xs px-3 py-1.5 rounded-lg flex-1">حذف</button>
          </div>
        </div>
      ))}
    </div>
...
export default function AdminPage() {
  ...
  return (
    <div style={{background:"#080808", minHeight:"100vh"}} dir="rtl" className="pb-20">
      <header style={{background:"#080808", borderBottom:"1px solid #141414"}} className="px-6 py-4 sticky top-0 z-10 flex items-center gap-2">
